...
async function processGeoData(mapData, config) {
  ...
  // 解构数据对象
  const { type: dataType, data: rawData, simplify: simplifyOptions } = mapData;
  // 合并默认配置
  const { useProject: enableProjection, useProcess: enableProcessing } = af(
    DEFAULT_CONFIG,
    config
  );
  ...
  if (enableProjection) {
    mapData.__geojson_process_proj__ = window.sm(
