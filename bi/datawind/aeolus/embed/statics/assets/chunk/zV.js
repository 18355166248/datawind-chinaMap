 ...
 */
function createDistrictInnerShadow(mapInstance) {
  // 获取内阴影样式配置
  const {
  ...
  if (
    ...
    const shadowTexture = (function generateShadowTexture(options) {
      ...
      // 初始化地图视图
      const mapView = window.MV({
        tileUrl: "",
        bbox: boundingBox,
        maxZoom: maxZoomLevel,
        worldBboxSize: window.Vf,
      ...
      for (
        ...
        // 计算特征的像素尺寸
        const featureWidth = Math.abs(
          featureTopLeft[0] - featureBottomRight[0]
        );
...
// 将函数暴露到全局作用域
window.zV = createDistrictInnerShadow;
